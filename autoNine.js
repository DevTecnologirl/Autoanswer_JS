...
function isTargetAcceptButton(btn) {
  ...
  let foundInboxCard = false;
  let foundCardActions = false;

  let current = btn;
  while (current) {
    const tag = current.tagName?.toUpperCase();

    if (tag === 'SN-INBOX-CARD') foundInboxCard = true;
    if (tag === 'NOW-CARD-ACTIONS') foundCardActions = true;

    if (foundInboxCard && foundCardActions) return true;
...
console.log("=== AutoAnswer dentro de SN-INBOX-CARD e NOW-CARD-ACTIONS ===");
